...
def _drop_vendored_license_bloat(pkg_datas):
    ...
    directory under dist/. None of these files are ever imported or read
    by this app at runtime -- only the flat top-level dist-info files
    (METADATA, RECORD) matter for the importlib.metadata lookups libraries
    like transformers make -- so the deep vendored subtree is safe to drop
    without losing anything the app actually needs.
    """
    filtered = []
    for source, dest in pkg_datas:
        dest_str = str(dest)
